...
class PortmoneCardViewController: UIViewController {
    ...
    public func initCardPayment(payeeId: String,
                                phoneNumber: String,
                                billAmount: Double,
                                competition: @escaping (_ data: FinishPaymentsData) -> Void
    ) {
    ...
    public func initCardSaving(payeeId: String, competition: @escaping (_ data: FinishPaymentsData) -> Void) {
        self.resolver = PortmoneCardResolver(resolver: competition)
...
extension PortmoneCardViewController: PortmonePaymentPresenterDelegate {
    func didFinishPayment(bill: Bill?, error: Error?) {
        if error != nil {
            let data = FinishPaymentsData(token: "")
            self.resolver?.onPaymentFinish(data)
            self.resolver = nil
        }
